    ...
    <View style={styles.screenView}>
      <View style={ styles.contentContainer }>
        ...
        <ScrollView style={styles.cinemaDetail}>
          ...
          <Text style={styles.info}>Address: {cinema['address\t']}, {cinema.city}</Text>
          ...
          <View>
            {movies.moviesAreLoading
              ...
              : movies.movies.length === 0
                ...
                : movies.moviesError
                  ? (<Text style={styles.loading}>Error occurred: {movies.moviesError}</Text>)
                  : (
                    <>
                      {movies.movies.map((movie, index) => (
                        <Movie key={index} movieData={movie} isLastItem={index === movies.movies.length - 1} />
                      ))}
                    </>
                    )}
          </View>
